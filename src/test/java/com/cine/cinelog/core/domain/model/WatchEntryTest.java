...
class WatchEntryTest {
        ...
        void constructor_rejectsRatingOutOfRange_and_acceptsBounds() {
                ...
                // negative rating
                assertThrows(IllegalArgumentException.class, () -> new WatchEntry(
                                1L, 2L, 3L, null, new BigDecimal("-1"), null, LocalDate.now(), now, now, null, null,
                                null));
                // greater than 10
                assertThrows(IllegalArgumentException.class, () -> new WatchEntry(
                                1L, 2L, 3L, null, new BigDecimal("10.1"), null, LocalDate.now(), now, now, null, null,
                                null));

                // boundaries 0 and 10 are allowed
                WatchEntry zero = new WatchEntry(1L, 2L, 3L, null, BigDecimal.ZERO, null, LocalDate.now(), now, now,
                                null, null,
                                null);
                WatchEntry ten = new WatchEntry(2L, 2L, 3L, null, BigDecimal.TEN, null, LocalDate.now(), now, now, null,
                                null,
                                null);
        ...
        void withId_returnsNewInstanceWithGivenId() {
                LocalDateTime now = LocalDateTime.now();
                WatchEntry original = new WatchEntry(1L, 2L, 3L, null, BigDecimal.ONE, "c", LocalDate.now(), now, now,
                                null,
                                null, null);
        ...
        void equalsAndHashCode_basedOnlyOnId() {
                LocalDateTime now = LocalDateTime.now();
                WatchEntry a = new WatchEntry(5L, 10L, 20L, null, BigDecimal.ONE, "a", LocalDate.now(), now, now, null,
                                null,
                                null);
                WatchEntry b = new WatchEntry(5L, 99L, null, 200L, BigDecimal.TEN, "different", LocalDate.now(), now,
                                now, null,
                                null, null);
                ...
                WatchEntry c = new WatchEntry(6L, 10L, 20L, null, BigDecimal.ONE, "a", LocalDate.now(), now, now, null,
                                null,
                                null);
                assertNotEquals(a, c);
        }
        ...
        void applyRating_setsRating_trimsComment_and_updatesUpdatedAt() {
                ...
                WatchEntry w = new WatchEntry(1L, 2L, 3L, null, null, "old", LocalDate.now(), created, previousUpdated,
                                null,
                                null, null);

                // Transition to COMPLETED state so rating can be applied
                w.startWatching(); // PLANNING → WATCHING
                w.markAsCompleted(null); // WATCHING → COMPLETED
                ...
                assertTrue(w.getUpdatedAt().isAfter(previousUpdated)
                                || w.getUpdatedAt().isEqual(previousUpdated) == false);
        }
