...
SELECT id, title, release_year FROM media WHERE type = 'SERIES';
...
INSERT INTO tmp_series_season_count (title, total_seasons) VALUES
('Breaking Bad', 5),
('Game of Thrones', 8),
('The Sopranos', 6),
('The Wire', 5),
...
-- 3) gerar seasons
INSERT INTO seasons (media_id, season_number, name, air_date)
SELECT
  m.id,
  s.season_number,
...
JOIN (
  SELECT 1 AS season_number UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL
...
SELECT id AS season_id, media_id, season_number, air_date
FROM seasons;
...
SELECT
  t.season_id,
  e.episode_number,
  CONCAT('Episode ', e.episode_number, ' (S', t.season_number, ')'),
  DATE_ADD(t.air_date, INTERVAL (7 * (e.episode_number - 1)) DAY)
FROM tmp_seasons t
JOIN (
  SELECT 1 AS episode_number UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL
  SELECT 4 UNION ALL SELECT 5 UNION ALL SELECT 6 UNION ALL SELECT 7 UNION ALL
  SELECT 8 UNION ALL SELECT 9 UNION ALL SELECT 10 UNION ALL SELECT 11 UNION ALL
...
WHERE e.episode_number <=
  CASE
    WHEN t.season_number = 1 AND t.media_id IN (SELECT id FROM media WHERE title IN ('Chernobyl','Band of Brothers','The Queen''s Gambit','Death Note','Cowboy Bebop','Fullmetal Alchemist: Brotherhood'))
      THEN 6
    WHEN t.media_id IN (SELECT id FROM media WHERE title='Friends') THEN 24
    ELSE 10
  END;
...
SELECT 'seasons_total' AS what, COUNT(*) FROM seasons
UNION ALL SELECT 'episodes_total', COUNT(*) FROM episodes
ORDER BY what;
SELECT m.title, s.season_number, COUNT(e.id) AS episodes
FROM media m
