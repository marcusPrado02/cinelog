...
INSERT INTO tmp_media_genres_map (title, g1, g2, g3) VALUES
-- FILMES
...
('The Lord of the Rings: The Fellowship of the Ring','Adventure','Fantasy','Drama'),
('The Lord of the Rings: The Return of the King','Adventure','Fantasy','War'),
...
('The Silence of the Lambs','Crime','Thriller','Drama'),
...
('Back to the Future','Adventure','Science Fiction','Comedy'),
('Terminator 2: Judgment Day','Action','Science Fiction','Thriller'),
...
('Dune: Part Two','Science Fiction','Adventure','Drama'),
('Oppenheimer','Drama','History','Biography'),
('Barbie','Comedy','Fantasy',NULL),
('La La Land','Romance','Music','Drama'),
('The Grand Budapest Hotel','Comedy','Drama','Crime'),
('Her','Romance','Science Fiction','Drama'),
('Blade Runner 2049','Science Fiction','Drama','Mystery'),
...
('Crouching Tiger, Hidden Dragon','Action','Romance','Fantasy'),
('The Truman Show','Drama','Science Fiction',NULL),
('The Big Lebowski','Comedy','Crime',NULL),
('The Incredibles','Animation','Action','Family'),
('WALL·E','Animation','Science Fiction','Family'),
('Up','Animation','Adventure','Family'),
('The Dark Knight Rises','Action','Crime','Superhero'),
('Guardians of the Galaxy','Action','Adventure','Science Fiction'),
('Spider-Man: Into the Spider-Verse','Animation','Action','Superhero'),
('Spider-Man: Across the Spider-Verse','Animation','Action','Superhero'),
('The Batman','Action','Crime','Superhero'),
-- SÉRIES
('Breaking Bad','Crime','Drama','Thriller'),
('Game of Thrones','Drama','Fantasy','Adventure'),
...
('The Mandalorian','Science Fiction','Adventure','Action'),
('The Last of Us','Drama','Horror','Science Fiction'),
...
('Westworld','Science Fiction','Drama','Mystery'),
...
('Severance','Science Fiction','Drama','Mystery'),
('The Queen''s Gambit','Drama','Sport',NULL),
('The Witcher','Fantasy','Action','Adventure'),
('Vikings','Drama','History','Action'),
('The Handmaid''s Tale','Drama','Science Fiction',NULL),
('Money Heist (La Casa de Papel)','Crime','Drama','Thriller'),
('Attack on Titan','Animation','Action','Fantasy'),
('Death Note','Animation','Mystery','Thriller'),
('Fullmetal Alchemist: Brotherhood','Animation','Action','Adventure'),
('One Piece','Animation','Action','Adventure'),
('Avatar: The Last Airbender','Animation','Adventure','Family'),
('Cowboy Bebop','Animation','Action','Science Fiction'),
...
SELECT title, g1 FROM tmp_media_genres_map;

-- g2 (somente não nulos)
INSERT IGNORE INTO tmp_media_genres_expanded (title, g)
SELECT title, g2 FROM tmp_media_genres_map WHERE g2 IS NOT NULL;

-- g3 (somente não nulos)
INSERT IGNORE INTO tmp_media_genres_expanded (title, g)
SELECT title, g3 FROM tmp_media_genres_map WHERE g3 IS NOT NULL;

-- ====== INSERÇÃO FINAL em media_genres ======
INSERT IGNORE INTO media_genres (media_id, genre_id)
SELECT m.id, g.id
FROM tmp_media_genres_expanded x
...
SELECT 'media_genres' AS what, COUNT(*) AS total FROM media_genres;
SELECT m.title, GROUP_CONCAT(g.name ORDER BY g.name SEPARATOR ', ') AS genres
FROM media m
